...
public class ConsulterController implements Initializable {
   ...
   private void actualiser() {
	   String query="select a.id_achat,a.date_achat,a.livraison,CONCAT(p.nom,' ',p.prenom) as fournisseur, ROUND(sum(c.prix_unitaire*c.nbr_caisse*c.longueur*c.largeur),2) as montant, ROUND(sum(nbr_caisse*c.longueur*c.largeur),2) as qte  "
          		+ "from achat a,personnes p ,caisse_matiere c \r\n"
          		+ "where a.id_fournisseur=p.id_personne and p.id_type=1 and a.id_achat = c.id_achat and a.type_achat=0   \r\n"
          		+ "group by a.id_achat,a.date_achat,a.livraison,fournisseur";
	   try {
		view.actualiser(data,0,query,connection);
      ...
      private void Ordre(Button click) {
    	click.setOnMouseClicked((MouseEvent event) -> {
    		...
    		if(a == null) {
    			...
    			loader.setLocation(getClass().getResource("/view/achat/matierePremier/ordre.fxml"));
    			try {
    				loader.load();
    ...
    private void Versement(Button click) {
    	click.setOnMouseClicked((MouseEvent event) -> {
    		...
    		if(a == null) {
    			...
    			FXMLLoader loader = new FXMLLoader ();
    			loader.setLocation(getClass().getResource("/view/achat/matierePremier/versement.fxml"));
    			try {
    				loader.load();
    			} catch (IOException ex) {
    				Logger.getLogger(ConsulterController.class.getName()).log(Level.SEVERE, null, ex);
